...
<body class="bg-gray-100 ">  
      ...
      <div class="swiper-container">
          <div class="swiper-wrapper">
              @foreach ($images as $image)
                  <div class="swiper-slide overflow-hidden" style="background-image: url('{{ asset('storage/app/public/' . $image->carousell) }}');">
                  </div>
              @endforeach
          </div>
    ...
    <section class="container mx-auto py-12 px-4 md:px-12 font-inter" data-aos="fade-right">
      ...
      <div class="flex flex-col gap-6">
          @foreach($jadwals as $jadwal)
              <div class="flex flex-col md:flex-row overflow-hidden md:mb-[60px]">
                  <img src="{{ asset('storage/app/public/' . $jadwal->image) }}" alt="Event"
                      class="w-full md:w-[600px] h-48 md:h-[350px] rounded-md object-cover">
                  <div class="p-2 md:ms-6 flex-1">
                      <p class="font-semibold font-rethink text-base md:text-lg">{{ $jadwal->name }}</p>
                      <p class="text-black mt-2 font-black">{{ $jadwal->description }}</p>
                      <a href="/acara-mendatang">
                        <button class="md:mt-8 mt-4 px-4 py-2 font-bold bg-black border-black border-2 duration-150 text-white rounded-[25px] hover:bg-white hover:text-black">
                          Lihat Selengkapnya
                        </button>
                      </a>                      
                  </div>
