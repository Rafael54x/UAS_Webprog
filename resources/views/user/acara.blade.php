...
<html lang="en">
  ...
  <body class="bg-grey">
    ...
    <div>
      ...
      <h1 class="mt-12 font-inter font-extrabold text-[50px] text-center">Acara Rutin</h1>
    </div>

   <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:mx-[20px] mt-12">
    @foreach($jadwals as $jadwal)
    <div class="relative max-w-xs mx-auto min-h-[200px] gap-4">
        <img src="{{ asset('storage/app/public/' . $jadwal->image) }}" alt="{{ $jadwal->name }}" class="w-full h-full hover:scale-95 ease-out duration-200">
        <div class="absolute bottom-4 left-4 p-2 rounded-md">
            <p class="font-inter font-bold text-xl text-white">{{ $jadwal->name }}</p>
            <p class="font-rethink text-lg text-white">{{ $jadwal->description }}</p>
            <p class="font-rethink text- text-white">{{ $jadwal->place }}</p>
        </div>
    </div>
    @endforeach
</div>
    ...
    <div class="mx-32 mt-12 mb-10">
      <table class="w-full text-left font-rethink shadow-lg overflow-hidden rounded-[25px]">
        <thead class="bg-[#737373] text-white text-lg" style="background-color: #737373;">
          <tr>
